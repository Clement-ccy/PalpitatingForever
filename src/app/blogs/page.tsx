...
export default function BlogsPage() {
    ...
    return (
        <div className="min-h-screen pt-32 px-4 pb-32 max-w-7xl mx-auto flex flex-col md:flex-row gap-8">
                ...
                <div className="relative border-l border-white/10 ml-3 space-y-8 pl-8 py-2">
                    {MOCK_BLOGS.map((post, index) => (
                        <div key={post.id} className="relative group">
                            {/* Timeline Dot */}
                            <div className={cn(
                                "absolute -left-[39px] top-6 w-5 h-5 rounded-full border border-white/20 transition-all flex items-center justify-center bg-[#0A0A0A] z-10",
                                ...
                                <div className={cn(
                                    ...
                                    selectedSlug === post.slug ? "bg-green-500" : "bg-white/20 group-hover:bg-green-500"
                                )} />
                            </div>
                            ...
                            <div onClick={() => setSelectedSlug(post.slug)} className="cursor-pointer">
                                <SpotlightCard className={cn(
                                    ...
                                    <h2 className={cn(
                                        ...
                                        selectedSlug === post.slug ? "text-green-400" : "group-hover:text-green-400"
                                    )}>
                                        {post.title}
                                    </h2>
            ...
            {/* Reading Pane (Right) - Desktop Only for "Split View" effect, or Overlay on Mobile */}
            <div className="hidden md:block w-full md:w-2/3 sticky top-32 h-[calc(100vh-10rem)] overflow-hidden rounded-2xl border border-white/10 bg-[#0F0F0F] relative">
                                ...
                                <div className="mb-8 border-b border-white/10 pb-8">
                                    <div className="flex gap-2 mb-6">
                                        {selectedPost.tags.map(tag => (
                                            <span key={tag} className="px-2 py-1 rounded-md bg-white/5 border border-white/5 text-xs text-white/60 font-mono">
                                                {tag}
                                            </span>
                                        ))}
                                    </div>
                                    <h1 className="text-4xl md:text-5xl font-black text-white mb-4 leading-tight">{selectedPost.title}</h1>
                                    <p className="text-xl text-white/60 leading-relaxed font-light">{selectedPost.summary}</p>
                                </div>
                                
                                {/* Prose Content (Styled Markdown) */}
                                <article className="prose prose-invert prose-lg max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-a:text-green-400 hover:prose-a:text-green-300 prose-code:text-green-300 prose-code:bg-white/5 prose-code:px-1 prose-code:rounded prose-pre:bg-[#050505] prose-pre:border prose-pre:border-white/10">
